...
#pragma region SHADER AND TEXTURE 
	...
	Shader objectShader("shader/modelviewvertex.glsl", "shader/multilightfragment.glsl");
	Shader lightShader("shader/lightsourcevertex.glsl", "shader/lightsourcefragment.glsl");
...
#pragma region RENDER LOOP
	...
	while (!glfwWindowShouldClose(window)) {
		...
		lights[2].direction = glm::vec3(view * glm::vec4(-1.0f, -1.0f, 0.0f, 0.0f));
			   
		lights[2].color = glm::vec3(1);
		lights[2].ambient = glm::vec3(0.1f);
		lights[2].diffuse = glm::vec3(0.8f);
		lights[2].specular = glm::vec3(1.0f);


		material.ambient = YELLOW_LIGHT;
		material.diffuse = diffuseMap.TextureUnit;
		material.specular = specularMap.TextureUnit;
		material.shininess = 32.0f;
		material.emission = emissionMap.TextureUnit;
